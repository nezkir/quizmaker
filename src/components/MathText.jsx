    ...
    if (c === 'frac') {
      ...
      return [<span key={kk()} style={{
        ...
        <span style={{
          ...
          fontSize:'0.82em', lineHeight:1.2, textAlign:'center', minWidth:'1em',
        }}>{num}</span>
        <span style={{
          paddingTop:2, fontSize:'0.82em', lineHeight:1.2,
          textAlign:'center', minWidth:'1em',
        }}>{den}</span>
      </span>, p3];
    }
    if (c === 'sqrt') {
      const [body, p2] = parseGroup(toks, pos);
      return [<span key={kk()} style={{display:'inline-flex',alignItems:'center',verticalAlign:'middle'}}>
        <span style={{fontSize:'1.2em',marginRight:1,lineHeight:1}}>√</span>
        <span style={{borderTop:'1.5px solid currentColor',paddingTop:1,paddingLeft:1}}>{body}</span>
      </span>, p2];
    }
